...
#### Scenario: CLI load does not begin before validation succeeds
- **WHEN** the CLI is invoked in Neo4j load mode and YAML validation fails
- **THEN** the loading workflow does not clear the target database and does not begin Neo4j loading

#### Scenario: CLI load requires confirmation or explicit bypass
- **WHEN** the CLI is invoked in Neo4j load mode after successful validation
- **THEN** the loading workflow proceeds only after interactive confirmation is accepted or an explicit force flag is provided
...
#### Scenario: CLI constructs Neo4j visitor after config and confirmation policy succeed
- **WHEN** the CLI prepares to begin Neo4j loading after successful validation
- **THEN** it constructs the Neo4j service and Neo4j visitor using successfully loaded runtime configuration and only after confirmation requirements have been satisfied
